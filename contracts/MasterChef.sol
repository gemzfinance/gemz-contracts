...
contract MasterChef is Ownable {
    ...
    // The EGG TOKEN!
    GemzToken public gemz;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public gemzPerBlock;
    // Bonus muliplier for early gemz makers.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _gemzPerBlock,
        uint256 _startBlock
    ) public {
        gemz = _gemz;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        gemzPerBlock = _gemzPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 gemzReward = multiplier.mul(gemzPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(gemzReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 gemzReward = multiplier.mul(gemzPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        gemz.mint(devaddr, gemzReward.div(10));
        gemz.mint(address(this), gemzReward);
        pool.accEggPerShare = pool.accEggPerShare.add(gemzReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe gemz transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 gemzBal = gemz.balanceOf(address(this));
        if (_amount > gemzBal) {
            gemz.transfer(_to, gemzBal);
        } else {
            gemz.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _gemzPerBlock) public onlyOwner {
        massUpdatePools();
        gemzPerBlock = _gemzPerBlock;
    }
}
